...
class AuthService {
	async registration(first_name, last_name, age, school, login, password) {
		const candidate = await userModel.findOne({ login });
		if (candidate) {
			throw new Error(`Пользователь с логином - ${login} уже существует`);
		}
		const hashPassword = bcrypt.hashSync(password, 7);
		const user = await userModel({ first_name, last_name, age, school, login, password: hashPassword });
